...
const generate = async () => {
  ...
  for (let index = 0; index < ExportData.value.length; index += 2) {
    ...
    if (index == ExportData.value.length - 1) {
      addRow([await createTableCell(element.title, 'title')])

      addRow([await createTableCell(element.picture, 'picture')])

      addRow([await createTableCell(element.content, 'content')])
    } else {
      addRow([
  ...
  const remainder = ExportData.value.length % 4
  if (Item.rows.length == remainder) {
    table = new Table(Item)
    let sections_item = {
      headers: {
        default: new Header({
